...
function jsonHeaders(csrf?: CsrfContext): Record<string, string> {
...
export async function fetchWorkbenchBootstrap(): Promise<WorkbenchBootstrap> {
  ...
  const payload = await parseJson<WorkbenchBootstrap>(response);
...
export async function fetchCaseList(filters: CaseFilters): Promise<ReconCaseList> {
  const response = await fetch(`/api/recon/cases${buildQuery(filters)}`, {
    credentials: 'same-origin',
    headers: jsonHeaders()
  });
  const payload = await parseJson<ReconCaseList>(response);
...
export async function fetchCaseDetail(caseId: string): Promise<ReconCaseDetail> {
  const response = await fetch(`/api/recon/cases/${caseId}`, {
    credentials: 'same-origin',
    headers: jsonHeaders()
  });
  return parseJson<ReconCaseDetail>(response);
}

export async function updateCaseStatus(caseId: string, status: ReconCaseStatus, csrf: CsrfContext): Promise<void> {
...
export async function createCaseNote(caseId: string, body: string, csrf: CsrfContext): Promise<void> {
